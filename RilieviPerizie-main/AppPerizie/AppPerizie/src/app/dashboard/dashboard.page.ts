...
export class DashboardPage {
  ...
  async cambiaPassword(event: Event) {
    ...
    const url = 'https://rilieviperizie.onrender.com/api/cambia-password';
    const body = {
  ...
  async onSubmit(event: Event) {
    ...
    try {
      ...
      const url = 'https://rilieviperizie.onrender.com/api/upload-perizia';

      const body = {
        descrizione: this.descrizione,
        foto: this.fotoSelezionate,
        coordinate: this.coordinate || { latitudine: 0, longitudine: 0 }, 
        dataOra: this.dataOra || new Date().toISOString(),
        codiceOperatore: this.codiceOperatore || 'Sconosciuto', 
      };
